...
PageUserTransaction::PageUserTransaction()
{
}
...
void PageUserTransaction::main()
{
	ManageAccount a;
	
	BasePage::header();
	string title = "LICH SU GIAO DICH " + a.FindByUserID(userID).getLastName() + a.FindByUserID(userID).getFirstName();
	BasePage::setTitlePage(title);
	BasePage::main();
}

void PageUserTransaction::setUI()
{
	box(0, 0, 64, 21);
...
void PageUserTransaction::handle()
{
	while (true)
	{
		ManageTransaction t;
...
void PageUserTransaction::paging()
{
	ManageTransaction t;
	PagingData<TransactionModel> page(t.GetAllTransaction(userID), 5);
...
void PageUserTransaction::cleanDataUI()
{
	int posX = 3;
	int posY = PADDING_TOP + 5;
